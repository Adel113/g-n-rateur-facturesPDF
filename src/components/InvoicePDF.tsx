...
export default function InvoicePDF({ invoiceId }: { invoiceId: string }) {
  ...
  return (
    <div className="space-y-6 font-sans text-black" style={{ fontFamily: "'Arial', sans-serif" }}>
      ...
      <div ref={printRef} id="invoice-content" className="bg-white" style={{ maxWidth: 595, margin: '0 auto', padding: 20, fontSize: 12, lineHeight: 1.2, color: '#000', minHeight: '842px', display: 'flex', flexDirection: 'column' }}>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', marginBottom: 20, fontSize: 10,  marginTop: 60 }}>
          <thead style={{ backgroundColor: '#333', color: '#fff' }}>
            <tr>
              <th style={{ border: '1px solid #333', padding: 5, textAlign: 'left' }}>Description :</th>
              <th style={{ border: '1px solid #333', padding: 5, textAlign: 'center' }}>Détails</th>
          ...
          <tbody>
            {invoice.items.map((item, index) => (
              <tr key={item.id} style={{ backgroundColor: index % 2 === 0 ? '#f9f9f9' : '#fff' }}>
                ...
                <td style={{ border: '1px solid #ccc', padding: 5, textAlign: 'center' }}>{item.details || ''}</td>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 20 }}>
            ...
            <div style={{ width: '45%', textAlign: 'right', backgroundColor: '#f0f8ff', padding: 10, border: '1px solid #ccc' }}>
              <div style={{ fontSize: 14, fontWeight: 'bold' }}>
                <div>TOTAL HT : <span style={{ marginLeft: 20 }}>{invoice.subtotal.toFixed(2)}€</span></div>
        ...
        {invoice.notes && (
          <div style={{ fontSize: 8, marginBottom: 20, padding: 10, borderTop: '1px solid #ccc', backgroundColor: '#fffacd' }}>
            <p style={{ whiteSpace: 'pre-line' }}>{invoice.notes}TVA non applicable selon l'article 293B du code du C.G.I. Dispensé d’immatriculation au registre du commerce et des sociétés (RCS) et au
 répertoire des métiers (RM). Conformément aux dispositions de l’article L441-6 du Code de Commerce, le non respect du délais de paiement de 15 jours
 entrainera facturation au client d’un intérêt de retard calculé par application à l’intégralité des sommes restant dues, d’un taux d’intérêt égal au taux d’intérêt
 appliqué par la Banque Centrale Européenne à son opération de refinancement la plus récente à la date d’échéance, ce taux étant majoré de dix points. En outre
 une indemnité forfaitaire pour frais de recouvrement, d'un montant de 40 euros sera due, de plein droit et sans notification préalable par le Client en cas de
 retard de paiement. Le Fournisseur se réserve le droit de demander au Client une indemnisation complémentaire si les frais de recouvrement effectivement
 engagés dépassaient ce montant, sur présentation des justificatifs.(Article L 441-6 du code de commerce). </p>
          </div>
        )}
          <div style={{ backgroundColor: '#333', color: '#fff', padding: 10, fontSize: 10, width: '555px' }}>
